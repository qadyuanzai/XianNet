...
void XianNet::CheckAndWeakUpWorker() {
  /*
  这段代码还有一点需要注意，多个线程可能同时访问sleepCount，理论上需要加锁，但为了节省加锁的开销，此处直接读取sleepCount。由于没加锁，sleepCount的值可能不准确，尽管出现的概率较小，但不排除某次需要唤醒时没有唤醒（因为sleepCount的值小了），或不该唤醒时唤醒了（sleepCount的值大了）。但这两种错误无伤大雅，若出现“需要唤醒时没有唤醒”的情况，等待下一次唤醒即可（后续还会加上Socket模块，接收数据时也要唤醒工作线程）；“不该唤醒时唤醒了”更没关系，就当工作线程空转一次。综合起来，我们认为错误唤醒的开销比给sleepCount加锁的开销小，于是宁愿偶尔出错也不加锁。
  */
  // unsafe
  if(sleep_count_ == 0) {
      return;
